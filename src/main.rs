...
fn main() {
    ...
    match audit_file(&file, options) {
        Ok(result) => {
            ...
            let has_errors = result.issues.iter()
        ...
        for file in &files {
            if let Ok(metadata) = fs::metadata(file) {
                if let Ok(modified) = metadata.modified() {
                    let changed = last_modified.get(file)
...
fn audit_all_workflows(options: AuditOptions, format: &str, strict: bool) {
        ...
        match audit_file(file, opts) {
            Ok(result) => {
                if format == "json" {
                    ...
                    let errors = result.issues.iter().filter(|i| i.severity == pipecheck::Severity::Error).count();
                    let warnings = result.issues.iter().filter(|i| i.severity == pipecheck::Severity::Warning).count();
    ...
    if format != "json" {
        println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        println!("Total: {} errors, {} warnings across {} files", total_errors, total_warnings, all_files.len());
    }
